...
def is_move_ok(grid, x_from, y_from, x_to, y_to):
    ...
    False
    >>> is_move_ok(grid, 0, 0, 0, -1) # right OOB
    False
    ...
    True
    >>> is_move_ok(grid, 1, 0, 2, 0)  # right ok
    False
    >>> is_move_ok(grid, 1, 0, 2, 1)  # right down
    False
    >>> is_move_ok(grid, 1, 0, 0, 1)  # left down
    True
    ...
    if not grid.in_bounds(x_to, y_to):
        ...
        return False
    elif grid.in_bounds(x_to, y_to -1) and grid.get(x_to, y_to -1)!=None and x_to+y_to !=x_from+y_from+1:
        return False
    """
    the reason why I put x_to+y_to !=x_from+y_from+1 is to exclude the cases when 's' moves right/left/down
    """
    return True
...
def do_gravity(grid, x, y):
    ...
    if grid.get(x, y) != 's':
        pass
    elif is_move_ok(grid, x, y, x, y + 1) == True:  # checks if the move to the bottom square is okay
        do_move(grid, x, y, x, y + 1)
    elif is_move_ok(grid, x, y, x - 1, y + 1) == True:  # checks if the move to the bottom-left square is okay
        do_move(grid, x, y, x - 1, y + 1)
    elif is_move_ok(grid, x, y, x + 1, y + 1) == True:  # checks if the move to the bottom-right square is okay
        do_move(grid, x, y, x + 1, y + 1)
    return grid
...
def do_brownian(grid, x, y, brownian):
    ...
    if grid.get(x, y) == 's':
        ...
        if num < brownian:
            ...
            if coin == 0:
                if is_move_ok(grid, x, y, x - 1, y) == True:
