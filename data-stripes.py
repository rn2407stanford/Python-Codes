...
def draw_stripes(width, height, fracs, title):
    ...
    for i in range(len(fracs)):
        canvas.fill_rect(0 + i*rect_width, 0, rect_width, height, color=(BASE + fracs[i]*DELTA, BASE, BASE))
    """
    for i in range(len(fracs)):
        canvas.fill_rect(0 + i * rect_width, 0, rect_width, height,
                         color=(BASE + fracs[i] * DELTA, BASE, BASE - fracs[i] * DELTA))
