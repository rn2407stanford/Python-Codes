...
def index_to_x(width, year_index):
    ...
    """
    sub_width=(width-2*SPACE)/len(YEARS)
    x= SPACE + year_index*sub_width
    return x
...
def draw_fixed(canvas):
    ...
    for x in range(len(YEARS)):
        x_axis=index_to_x(width, x)
        canvas.create_line(x_axis, SPACE, x_axis, height - SPACE)
        canvas.create_text(x_axis, height - SPACE, text=YEARS[x], anchor=tkinter.NW, fill='black')
...
def draw_name(canvas, names, name, color):
    ...
    width = canvas.winfo_width()
    height = canvas.winfo_height()
    sub_height = (height-2*SPACE)/MAX_RANK # weighting as  rank and  height have different values
    n=len(YEARS)
    previous_x=index_to_x(width, 0)
    previous_y=best_rank(names, name, YEARS[0])
    for number in range(n):
        year=YEARS[number]
        x=index_to_x(width, number)
        y0=best_rank(names, name, year)
        y=sub_height*y0 + SPACE
...
def draw_names(canvas, names, lookups):
    ...
    # for part (d)
    for name in lookups:
        mod = lookups.index(name) % 4
        colors= COLORS[mod]
        draw_name(canvas, names, name, colors)
    return
